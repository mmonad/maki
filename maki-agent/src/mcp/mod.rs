...
/// One published MCP tool. Search text is derived from `definition` on
/// demand: searches are model-paced and rare, so nothing to cache.
...
/// or the `tool_search` catalog goes stale. Any tool the session touched
/// (searched, called, or called before a restart) is declared on the next
/// request.
#[derive(Clone)]
...
impl McpSession {
    ...
    pub fn extend_tools(&self, tools: &mut Value) {
        ...
        let defer = idx.descriptors.iter().filter(|d| !d.always_load).count() > idx.defer_tools;
        ...
        for d in idx.descriptors.iter() {
            ...
            if !defer || d.always_load || loaded.contains(&*d.qualified_name) {
                ...
                deferred.push(&d.wire_name);
            }
        }
        drop(loaded);
        if !deferred.is_empty() && !existing.contains(TOOL_SEARCH_TOOL_NAME) {
    ...
    pub fn search_tools(&self, query: &str) -> Result<String, String> {
        ...
        if hits.is_empty() {
            return Ok(format!(
                "{SEARCH_NO_MATCH} '{query}'. Keywords are matched \
                 case-insensitively against tool names, descriptions, and parameter \
                 names; try different keywords or an exact name from the catalog."
            ));
        }
        let mut out = format!(
            "Loaded {} tool(s), callable from your next message: {}",
            hits.len(),
            hits.join(", ")
        );
...
fn parse_entries(config: McpConfig) -> McpManagerInner {
    let defer_tools = config.defer_tools;
    ...
    McpManagerInner {
        entries,
        generation: 0,
        defer_tools: defer_tools.unwrap_or(DEFAULT_DEFER_TOOLS),
    }
}
    ...
    for entry in &inner.entries {
            ...
            && entry.status != McpServerStatus::Disabled
        {
            ...
            for t in &entry.tools {
                ...
                let wire_name = wire_tool_name(&t.qualified_name);
...
fn tool_search_definition(deferred: &[&str]) -> Value {
    json!({
        "name": TOOL_SEARCH_TOOL_NAME,
        "description": format!(
            "Load deferred MCP tools before using them. Deferred tools are not \
             callable until loaded through this tool: search once, then call the \
             loaded tools from your next message. Query keywords are matched \
             case-insensitively against tool names, descriptions, and parameter \
             names; the best matches load first, and an exact tool name always \
             wins.\n\n\
             Deferred tools: {}",
            deferred.join(", ")
        ),
        "input_schema": {
            ...
            "properties": {
                "query": {
                    "type": "string",
                    "description": "Keywords or an exact tool name, e.g. 'github issue' or 'linear__create_comment'"
                }
            },
            "required": ["query"]
        }
...
mod tests {
    ...
    #[test_case(None, DEFAULT_DEFER_TOOLS ; "default")]
    #[test_case(Some(3), 3 ; "configured")]
    fn parse_entries_resolves_defer_tools(configured: Option<usize>, expected: usize) {
        let mut config = make_config(vec![("srv", stdio_raw(&["x"]))]);
        config.defer_tools = configured;
        assert_eq!(parse_entries(config).defer_tools, expected);
    }
    ...
    fn extend_tools_defers_behind_tool_search_by_default() {
        ...
        assert!(catalog.contains(WIRE_TOOL_NAME), "catalog lists wire name");
    ...
    #[test]
    fn search_exact_name_outranks_keyword_matches() {
        ...
        let last = format!("srv__tool-{MAX_SEARCH_LOADS}");
        ...
        assert!(
            result.contains(&last) && !overflow.contains(&last),
    ...
    fn mid_session_loads_never_flip_remainder_into_context() {
        ...
        assert!(description.contains("srv__gamma"), "got: {description}");
    }
    ...
    fn existing_wire_name_stays_out_of_catalog() {
        ...
        assert!(catalog.contains("srv__beta"), "got: {catalog}");
        assert!(!catalog.contains("srv__alpha"), "got: {catalog}");
    }
